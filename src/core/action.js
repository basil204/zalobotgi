...
module.exports = (api) => {
  ...
  function handleReaction(event) {
    if (!Array.isArray(global.client.handleReaction)) {
      console.error(
        "global.client.handleReaction không phải là mảng:",
        global.client.handleReaction
      );
      return;
    }

    const reactionHandler = global.client.handleReaction.find(
    ...
    if (reactionHandler) {
      ...
      if (command && typeof command.handleReaction === "function") {
        ...
        console.error(
          "Command không tồn tại hoặc không có phương thức handleReaction:",
          reactionHandler.name
        );
      }
    } else {
      console.log(
        "Không tìm thấy reaction handler cho messageID:",
        event.messageID
      );
    }
  }
  ...
  function handleReply(event) {
    if (!Array.isArray(global.client.handleReply)) {
      console.error(
        "global.client.handleReply không phải là mảng:",
    ...
    if (replyHandler) {
      ...
      if (command && typeof command.handleReply === "function") {
        ...
        console.error(
          "Command không tồn tại hoặc không có phương thức handleReply:",
          replyHandler.name
        );
      }
    } else {
      console.log(
        "Không tìm thấy reply handler cho messageID:",
  ...
  function handleCommand(args) {
    ...
    if (args.length === 0 || typeof args[0] !== "string") {
      console.error(
        "Expected non-empty array with a string as the first element, got:",
        args
      );
      return;
    }
    ...
    if (args[0].startsWith("!")) {
      ...
      if (global.client.commands.has(command)) {
        ...
        api.sendMessage(
          `⛔ Lệnh bạn nhập không tồn tại!\n♟️ Lệnh gần giống nhất là: ${closestCommand}`,
          args[1] || event.threadID
        );
      }
    } else {
